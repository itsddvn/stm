...
- [ ] Import validation and target check
- [ ] Additive preselected import
- [ ] Secret/command rejection tests

## Success Criteria

- [ ] macOS-exported file cannot run on Windows/Linux.
- [ ] Importing a file with a custom URL does not create a managed command.
- [ ] MCP secret values never appear in exported bytes.
- [ ] Resources already installed as current become no-ops, not reinstalls.
