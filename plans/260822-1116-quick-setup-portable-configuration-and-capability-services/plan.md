...
- `tools-manager-core`: application, capabilities, domain, ports, catalog, inventory, lifecycle. No Tauri. No concrete OS/SQLite/keyring impl.
- `tools-manager-runtime`: adapters, storage, process, filesystem, network, credentials.
...
- Reconciled stale references: pending phase-file patches in this turn
- Unresolved contradictions: 0

## Current Gaps

- Tools page has no Quick Setup, select-all, or import.
- Batch planner is update-queue only.
- `managed_current` tools can still show Preview Managed Update.
- Recommended catalog is locked to ten ids and mostly macOS mappings.
- Candidate tools have empty mappings.
- Settings has no export/import or provider preference.
- Application and lifecycle services are god objects in one crate with infrastructure deps.
...
| 2 | [Core Runtime Split](./phase-02-core-runtime-split.md) | Pending |
| 3 | [Catalog Recipes And Providers](./phase-03-catalog-recipes-and-providers.md) | Pending |
| 4 | [Capabilities And Mixed Planning](./phase-04-capabilities-and-mixed-planning.md) | Pending |
| 5 | [Quick Setup And Settings](./phase-05-quick-setup-and-settings.md) | Pending |
| 6 | [Portable Configuration](./phase-06-portable-configuration.md) | Pending |
| 7 | [Verification And Contract Lock](./phase-07-verification-and-contract-lock.md) | Pending |

## Success Criteria

- [ ] Supported selected tools install from one review without the user choosing package-manager commands.
- [ ] Existing owners are preserved until an explicit reviewed migration.
- [ ] Homebrew and Bun bootstrap use pinned verified artifacts, never remote shell scripts.
- [ ] Portable import cannot execute file-supplied commands or expose secrets.
