...
pub fn catalog_inventory(catalog: &ToolCatalogSnapshot) -> InventorySnapshot {
    let platform = current_platform_slug();
    let platform_label = current_platform_label();
    let native_manager = current_native_linux_manager();
    ...
    let tools = catalog
        ...
        .iter()
        .map(|entry| {
            let Some(mapping) = mapping_for_platform(entry, platform, native_manager) else {
                return missing_tool(entry, platform_label, "mapping.unsupported");
            };
            reconcile_tool(
                entry,
                mapping,
