...
pub(super) fn prepare_skill(
    ...
    resolver: &dyn SkillSourceResolverPort,
    ...
    let receipts = store
        ...
        .into_iter()
        .filter(|(_, receipt)| receipt.skill_id == request.resource_id)
        .collect::<Vec<_>>();

    if action == "rollback" {
        let backups = store.load_available_skill_backups(&request.resource_id)?;
    ...
    let staging = resolver.resolve(workspace, &source, &cancel)?;
    ...
    if result.is_err() {
        let _ = resolver.cleanup(workspace, &staging);
    }
    result
}
