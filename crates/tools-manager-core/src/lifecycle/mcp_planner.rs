...
use crate::{
    ...
    mcp::{
        discover_mcp,
        lifecycle::{
            client_config_path, config_digest, McpConfigTarget, McpMutationAction,
            PreparedMcpAction, PreparedMcpMutation,
        },
    },
...
use super::{
    command::{compile_mcp_stdio, ExecutableIdentity},
    planner::{opaque_plan_id, plan_digest, prepare_review_only, PreparedPlan},
...
pub(super) fn prepare_mcp(
    ...
    if action == "rollback" {
        return prepare_rollback(workspace, request, sequence, now);
    }
    ...
    if matches!(action, "add" | "update" | "enable") {
        for candidate in &execution_servers {
            ...
            if candidate.transport == McpTransport::Stdio {
                match compile_mcp_stdio(&candidate.command_or_url, &candidate.args) {
                    Ok(Some(command)) => {
...
fn build_mutation_plan(
    ...
    for binding in bindings {
        ...
        let config_path = client_config_path(&home, &binding.client);
        targets.push(McpConfigTarget {
            client: binding.client,
            expected_sha256: config_digest(&config_path)?,
            config_path,
...
fn prepare_rollback(
    ...
    let backup_id = request
        ...
        .or_else(|| {
            store
                .load_available_mcp_backups(&request.resource_id)
                .ok()
    ...
    let backup = store
        .load_mcp_backup(&backup_id)?
        .ok_or_else(|| CoreError::LifecycleEvidenceChanged("MCP backup is unavailable".into()))?;
    let plan_id = opaque_plan_id(sequence, &request, now)?;
    let config_path = client_config_path(&workspace.skill_home()?, &backup.client);
    let issued_at = format_timestamp(now)?;
    let expires_at = format_timestamp(now + PLAN_TTL)?;
    let mut plan = LifecyclePlan {
        request: request.clone(),
