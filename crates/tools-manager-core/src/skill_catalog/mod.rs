    ...
    C: CatalogClock,
{
    ...
    pub fn refresh_remote(
        ...
        let state_path = skill_catalog_state_path(workspace_db_path);
        let persisted = persistence::read_state(&state_path).ok().and_then(|parts| {
            verify_snapshot(
                &parts,
                now,
                None,
        ...
        persistence::persist_state(&state_path, &snapshot)?;
        Ok(snapshot)
    }
    ...
    pub fn load_last_good_or_bundled(
        ...
        let now = self.clock.now_utc();
        let state_path = skill_catalog_state_path(workspace_db_path);
        let persisted_parts = persistence::read_state(&state_path);
    ...
    pub fn load(
        ...
        match self.refresh_remote(workspace_db_path, accepted) {
            Ok(snapshot) => Ok(snapshot),
            Err(remote_error) => self
                .load_last_good_or_bundled(workspace_db_path, accepted)
...
pub fn skill_catalog_state_path(workspace_db_path: &Path) -> std::path::PathBuf {
    workspace_db_path.with_file_name("skill-catalog-last-known-good.json")
}
...
pub fn load_current_authenticated_catalog(
    ...
    SkillCatalogService::new(OfflineCatalogRemote, SystemCatalogClock)
        .load_last_good_or_bundled(workspace_db_path, None)
}
