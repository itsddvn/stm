...
use crate::{
    skill_catalog::load_current_authenticated_catalog, skill_lifecycle::validate_staged_tree,
...
use stm_core::{
    ...
    skill_lifecycle::{
        ManagedSkillReceipt, SkillStagingEvidence, SkillTargetSpec, TreeValidationPolicy,
    },
...
pub fn scan_runtime_skills(
    ...
    let verified = load_current_authenticated_catalog(&workspace.db_path()).ok();
    if verified.is_none() {
        report.warnings.push(
            "Authenticated skill catalog is unavailable; managed skill actions remain blocked."
                .to_string(),
        );
    }
