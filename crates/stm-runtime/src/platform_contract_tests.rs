...
mod linux {
    use stm_core::{
        ...
        lifecycle::LifecycleExecutionPort,
        ports::HostExecutableResolver,
    };

    use crate::{linux::inspect_linux_manager, RealHostExecutableResolver, RealLifecycleExecutor};
    ...
    fn mapping(manager: &str, adapter: &str, package_id: &str) -> ToolCatalogMapping {
        ToolCatalogMapping {
            ...
            step: stm_core::domain::recipe::RecipeStepType::ManagerPackage,
    ...
    fn execute(mapping: &ToolCatalogMapping, action: &str, target: Option<&str>) {
        let command = RealHostExecutableResolver
            .compile_manager_command(mapping, action, target)
    ...
    fn exercise_install_rescan_noop_uninstall(mapping: ToolCatalogMapping) {
        ...
        let initial = inspect_linux_manager(&RealHostExecutableResolver, &mapping)
            .expect("initial manager evidence");
        if initial.installed {
            execute(&mapping, "uninstall", None);
        }

        let available = inspect_linux_manager(&RealHostExecutableResolver, &mapping)
            .expect("available manager evidence");
        assert!(!available.installed);
        execute(&mapping, "install", Some(&available.target_version));

        let installed = inspect_linux_manager(&RealHostExecutableResolver, &mapping)
            .expect("installed manager evidence");
        ...
        execute(&mapping, "uninstall", None);
        let removed = inspect_linux_manager(&RealHostExecutableResolver, &mapping)
            .expect("removed manager evidence");
    ...
    fn pacman_install_and_update_fail_closed_before_spawn() {
        ...
        assert!(RealHostExecutableResolver
            .compile_manager_command(&mapping, "install", Some("2.51.0"))
            .expect("Pacman install policy")
            .is_none());
        assert!(RealHostExecutableResolver
            .compile_manager_command(&mapping, "update", Some("2.51.0"))
...
mod macos {
    use stm_core::{
        ...
        domain::inventory::{ExecutionMode, MappingStatus, OwnershipKind, PrivilegeRequirement},
        feasibility::process_supervisor::CancelSignal,
        lifecycle::{LifecycleExecutionPort, ManagerEvidencePort, ManagerStateEvidence},
        ports::HostExecutableResolver,
    };

    use crate::{RealHostExecutableResolver, RealLifecycleExecutor, RealManagerEvidence};
    ...
    fn inspect(mapping: &ToolCatalogMapping) -> ManagerStateEvidence {
        let executable = RealHostExecutableResolver
            .manager_evidence_executable(mapping, "install")
            .expect("resolve manager")
            .expect("manager executable");
        RealManagerEvidence::new(std::sync::Arc::new(RealHostExecutableResolver))
