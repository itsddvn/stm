use stm_core::{
    catalog::ToolCatalogMapping, lifecycle::ManagerStateEvidence, ports::HostExecutableResolver,
    CoreError,
};

use crate::{host::RealHostExecutableResolver, manager_evidence::read_only_command};

pub(crate) fn inspect_linux_manager(
    ...
    match mapping.adapter.as_str() {
        "apt_package" => inspect_apt(host, mapping),
        "dnf_package" => inspect_dnf(host, mapping),
        "pacman_package" => inspect_pacman(host, mapping),
...
fn inspect_apt(
    ...
    let dpkg_query = required_executable(host, "dpkg-query")?;
    let apt_cache = required_executable(host, "apt-cache")?;
...
fn inspect_dnf(
    ...
    let rpm = required_executable(host, "rpm")?;
    let dnf = required_executable(host, "dnf")?;
    let installed_output = read_only_command(
        path_text(&rpm),
        vec![
            "-q".to_string(),
...
fn inspect_pacman(
    ...
    let pacman = required_executable(host, "pacman")?;
    let installed_output = read_only_command(
        path_text(&pacman),
...
fn required_executable(
    ...
    host.resolve_executable(name).ok_or_else(|| {
