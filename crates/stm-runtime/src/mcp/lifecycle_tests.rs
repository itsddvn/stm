...
use stm_core::{
    ...
    mcp::lifecycle::{
        McpBackupReceipt, McpBackupState, McpConfigTarget, McpMutationAction, McpRecoveryPhase,
        McpRecoveryRecord, McpTargetStatus, PreparedMcpMutation,
    },
    CoreError,
};
...
fn interrupted_replacement_is_rolled_back_before_recovery_journal_cleanup() {
    ...
    let backup = McpBackupReceipt {
        ...
        original_sha256: Some(
            stm_core::adapters::compute_sha256([original.to_vec()])
                .trim_start_matches("sha256:")
                .to_string(),
        ),
        replacement_sha256: stm_core::adapters::compute_sha256([replacement.to_vec()])
            .trim_start_matches("sha256:")
            .to_string(),
...
fn interrupted_recovery_rejects_user_changes_and_preserves_the_journal() {
    ...
    write_config(&backup_path, &encrypted);
    let digest = |bytes: &[u8]| {
        stm_core::adapters::compute_sha256([bytes.to_vec()])
            .trim_start_matches("sha256:")
            .to_string()
    };
    let backup = McpBackupReceipt {
