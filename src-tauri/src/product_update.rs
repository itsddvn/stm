...
impl ProductUpdateRuntime {
    ...
    pub async fn availability(&self, app: &AppHandle) -> ApplicationUpdateRecord {
        let current_version = app.package_info().version.to_string();
        match self.authenticated_update(app).await {
            Ok(Some((update, _))) => ApplicationUpdateRecord {
                ...
                selected: false,
                risk: "Authenticated separate signed product channel".into(),
            },
            Ok(None) | Err(_) => unavailable_product_update(
                current_version,
                if self.updater_enabled {
                    "No authenticated newer signed product release is available"
                } else {
                    "Unavailable in internal builds: signed product updater is not configured"
                },
            ),
        }
    }

    async fn authenticated_update(
        &self,
        app: &AppHandle,
    ) -> Result<Option<(tauri_plugin_updater::Update, VerifiedReleaseMetadata)>, String> {
        if !self.updater_enabled {
            return Ok(None);
        }
        let metadata = match verify_release_metadata(app).await {
        ...
        validate_authenticated_update(&update, &metadata)?;
    ...
    pub async fn prepare(
        ...
        let authenticated_update = self.authenticated_update(app).await?;
        let (current_version, target_version, identity, source, execution, confidence, limitations) =
            if let Some((update, metadata)) = authenticated_update {
    ...
    pub fn native_confirmation_summary(
        ...
        if locale.starts_with("en") {
            Ok(format!(
                "Update STM from {} to {}?\n\nSource: {}\nThe signed release will be revalidated before installation.",
                plan.plan.current_version, plan.plan.target_version, plan.plan.source
            ))
        } else {
            Ok(format!(
                "Cập nhật STM từ {} lên {}?\n\nNguồn: {}\nBản phát hành đã ký sẽ được kiểm tra lại trước khi cài đặt.",
                plan.plan.current_version, plan.plan.target_version, plan.plan.source
            ))
        }
    }
...
fn unavailable_product_update(current_version: String, risk: &str) -> ApplicationUpdateRecord {
    ApplicationUpdateRecord {
        id: "update-product".into(),
        resource_type: ApplicationUpdateKind::Product,
        name: "STM".into(),
        target: current_version.clone(),
...
fn validate_authenticated_update(
    ...
    {
        return Err("Signed product update endpoint is not credential-free HTTPS".into());
    }
...
mod tests {
    ...
    #[test]
    fn unavailable_product_update_is_typed_detect_only() {
        let update = unavailable_product_update(
            "0.1.0".into(),
            "Unavailable in internal builds: signed product updater is not configured",
        );

        assert_eq!(update.resource_type, ApplicationUpdateKind::Product);
        assert_eq!(update.execution_mode, UpdateExecutionMode::DetectOnly);
        assert_eq!(update.current, update.target);
        assert!(!update.selected);
        let view = stm_core::application::dto::UpdateViewModelDto::from(&update);
